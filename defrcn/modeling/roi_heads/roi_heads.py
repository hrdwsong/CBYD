...
class Res5ROIHeads(ROIHeads):
    ...
    # 执行该函数，将直接返回网络对所有proposal的预测结果，而不进行nms和test_score门限的过滤
    def fsl_forward(self, images, features, proposals, targets=None):
        del images
        # 如果targets不为空，则意味着是传入了support特征，直接使用gt_boxes映射ROI特征。
        ...
        return pred_class_logits, feature_pooled

    # 执行该函数后，特征图将不筛选出ROI特征，而是全特征图的前向映射
    def all_feat_map_forward(self, features):
        x = self.res5(features['res4'])
        return x
