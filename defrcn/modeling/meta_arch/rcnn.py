...
class GeneralizedRCNN(nn.Module):
    ...
    def fsl_forward(self, inputs, gt_instances=None):
        ...
        if self.cfg.MODEL.ROI_HEADS.ENABLE_DECOUPLE:
            scale = self.cfg.MODEL.ROI_HEADS.BACKWARD_SCALE
            features_de_rcnn = {k: self.affine_rcnn(decouple_layer(features[k], scale)) for k in features}
        results, roi_features = self.roi_heads.fsl_forward(images, features_de_rcnn, proposals, gt_instances)
        # 把proposal的显著度返回
        proposal_objectiveness = torch.cat([p.objectness_logits for p in proposals])
        return results, proposal_objectiveness, roi_features

    def fsl_feat_forward(self, inputs, gt_instances=None):
        images = self.preprocess_image(inputs)
        features = self.backbone(images.tensor)
        ...
        if self.cfg.MODEL.ROI_HEADS.ENABLE_DECOUPLE:
            scale = self.cfg.MODEL.ROI_HEADS.BACKWARD_SCALE
            features_de_rcnn = {k: self.affine_rcnn(decouple_layer(features[k], scale)) for k in features}
        feature_map = self.roi_heads.all_feat_map_forward(features_de_rcnn)
        return feature_map

    def support_forward(self, inputs, gt_instances=None):
        gt_instances = [inputs[0]['instances'].to(self.device)]
        images = self.preprocess_image(inputs)
        features = self.backbone(images.tensor)
