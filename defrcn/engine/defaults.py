...
from defrcn.evaluation import DatasetEvaluator, inference_on_dataset, print_csv_format, verify_results, fsl_inference
...
class DefaultTrainer(SimpleTrainer):
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            # 如果test数据集中包含fsl，则使用fsl评估方式，完成后直接返回
            # base训练完成后，则对整个base集进行评估
            if 'fsl' in cfg.DATASETS.TEST[0]:
                # 加载support set
                support_dataloader = cls.build_test_loader(cfg, cfg.DATASETS.TRAIN[0])
                acc = fsl_inference(model, support_dataloader, data_loader, cfg)

                return acc  # 此处直接返回，目标检测的evaluator不能用于miniIN
