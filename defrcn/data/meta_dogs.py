...
__all__ = ["register_meta_dogs"]


def load_dogs_json(json_file, image_root, metadata, dataset_name):
    ...
    if is_shots:
        ...
        if dataset_name.split('_')[-2] == 'support':
            json_file = os.path.join(json_file, 'dogs-split', '{}w{}s_episode{}'.format(n_way, n_shot, n_epi), 'support.json')
        else:
            json_file = os.path.join(json_file, 'dogs-split', '{}w{}s_episode{}'.format(n_way, n_shot, n_epi),
                                     'query.json')
...
def register_meta_dogs(name, metadata, imgdir, annofile):
    DatasetCatalog.register(
        name,
        lambda: load_dogs_json(annofile, imgdir, metadata, name),
    )
    ...
    MetadataCatalog.get(name).set(
        ...
        image_root=imgdir,
        evaluator_type="coco",
        dirname=r"datasets/datasets/dogs-coco",
        **metadata,
    )