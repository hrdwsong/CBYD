...
__all__ = ["register_meta_cars"]


def load_cars_json(json_file, image_root, metadata, dataset_name):
    ...
    if is_shots:
        ...
        if dataset_name.split('_')[-2] == 'support':
            json_file = os.path.join(json_file, 'cars-split', '{}w{}s_episode{}'.format(n_way, n_shot, n_epi), 'support.json')
        else:
            json_file = os.path.join(json_file, 'cars-split', '{}w{}s_episode{}'.format(n_way, n_shot, n_epi),
                                     'query.json')
...
def register_meta_cars(name, metadata, imgdir, annofile):
    DatasetCatalog.register(
        name,
        lambda: load_cars_json(annofile, imgdir, metadata, name),
    )
    ...
    MetadataCatalog.get(name).set(
        ...
        image_root=imgdir,
        evaluator_type="coco",
        dirname=r"datasets/datasets/cars-coco",
        **metadata,
    )